...
def main():
    ...
    # Collect messages to write as comments
    env_comments = []
    ...
    size_msg = f"Total environment file size: {env_length} bytes (system max: {max_env_size} bytes)"
    print(size_msg)
    env_comments.append(f"# {size_msg}")

    if env_length > max_env_size:
        warn1 = f"Warning: The environment file size ({env_length} bytes) exceeds the system's max allocatable size ({max_env_size} bytes)."
        warn2 = "Continuing may result in undefined behavior in some shells or applications."
        print(warn1)
        print(warn2)
        env_comments.append(f"# {warn1}")
        env_comments.append(f"# {warn2}")
        if os.name == "nt":
            note = "NOTE: Unknown if Windows limit applies to WSL/Docker."
            print(note)
            env_comments.append(f"# {note}")
    ...
    try:
        with open(output_filename, "w") as f:
            ...
            f.write(
                f"# Generated by Scaffold Environment Generator on {now}{os.linesep}"
            )
            f.write(
                f"# Do not edit this file directly; use the template instead.{os.linesep}"
            )
            f.write(os.linesep.join(env_comments) + os.linesep)
